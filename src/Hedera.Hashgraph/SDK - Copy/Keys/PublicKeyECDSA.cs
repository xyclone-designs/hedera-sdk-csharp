...
namespace Hedera.Hashgraph.SDK.Keys
{
    ...
    class PublicKeyECDSA : PublicKey
    {
		...
		public override EvmAddress ToEvmAddress()
        {
            ...
            byte[] uncompressed = Key.ECDSA_SECP256K1_CURVE.Curve.DecodePoint(ToBytesRaw()).GetEncoded(false);
            byte[] keccakBytes = BouncyCastleExtensions.CalcKeccak256(uncompressed[1..uncompressed.Length].CopyArray());
		...
		{
			byte[] hash = BouncyCastleExtensions.CalcKeccak256(message);
