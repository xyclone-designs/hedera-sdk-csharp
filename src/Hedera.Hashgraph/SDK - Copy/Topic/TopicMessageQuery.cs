    ...
    public sealed class TopicMessageQuery
    {
        ...
        { 
            ...
            private get => field ??= (Predicate<Exception>)((exception) => 
            {
				...
				{
					var code = rpcexception.Status.StatusCode;
					var description = rpcexception.GetStatus().Description;

					return 
                        (code == ResponseStatus.NOT_FOUND) ||
                        (code == ResponseStatus.UNAVAILABLE) || 
                        (code == ResponseStatus.RESOURCE_EXHAUSTED) || 
                        (code == ResponseStatus.Code.INTERNAL && description != null && Executable.RST_STREAM.Matcher(description).Matches());
				}
