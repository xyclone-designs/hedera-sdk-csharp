...
namespace Hedera.Hashgraph.SDK
{
    ...
    sealed class Crypto
    {
		...
		internal static ICipher InitAesCtr128(KeyParameter cipherKey, byte[] iv, bool forDecrypt)
        {
            ICipher aesICipher;
            try
            {
                aesICipher = ICipher.GetInstance("AES/CTR/NOPADDING");
            }
            ...
            return InitAesICipher(aesICipher, cipherKey, iv, forDecrypt);
        }
		...
		internal static ICipher InitAesCbc128Encrypt(KeyParameter cipherKey, byte[] iv)
        {
            ICipher aesICipher;
            try
            {
                aesICipher = ICipher.GetInstance("AES/CBC/NoPadding");
            }
            catch (NoSuchAlgorithmException e)
            {
            ...
            return InitAesICipher(aesICipher, cipherKey, iv, false);
        }
		...
		/// <returns>                         the aes cbc cipher</returns>
		internal static ICipher InitAesCbc128Decrypt(KeyParameter cipherKey, AlgorithmParameters parameters)
        {
            ICipher aesICipher;
            try
            {
            ...
            try
            {
                aesICipher.Init(ICipher.DECRYPT_MODE, new SecretKeySpec(cipherKey.GetKey(), 0, 16, "AES"), parameters);
            }
		...
		/// </summary>
		/// <param name="aesICipher">the aes cipher</param>
		/// <param name="cipherKey">the cipher key</param>
		/// <param name="iv">the initialization vector byte array</param>
		/// <param name="forDecrypt">is this for decryption True or encryption False</param>
		/// <returns>                         the new aes cipher</returns>
		internal private static ICipher InitAesICipher(ICipher aesICipher, KeyParameter cipherKey, byte[] iv, bool forDecrypt)
        {
            int mode = forDecrypt ? ICipher.DECRYPT_MODE : ICipher.ENCRYPT_MODE;
            try
            {
                aesICipher.Init(mode, new SecretKeySpec(cipherKey.GetKey(), 0, 16, "AES"), new IvParameterSpec(iv));
            }
            catch (InvalidKeyException e)
            {
                throw new Exception("platform does not support AES-128 ciphers", e);
        ...
        {
            ICipher aesICipher = InitAesCtr128(cipherKey, iv, false);
            return RunICipher(aesICipher, input);
        }
        ...
        {
            ICipher aesICipher = InitAesCtr128(cipherKey, iv, true);
            return RunICipher(aesICipher, input);
        }
		...
		internal static byte[] RunICipher(ICipher cipher, byte[] input)
        {
