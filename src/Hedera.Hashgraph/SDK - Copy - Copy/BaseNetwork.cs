...
namespace Hedera.Hashgraph.SDK
{
...

	{
		...
		public virtual Dictionary<string, IList<KeyT>> Network
		{
            private get;
			set
			{
		...
		} = new ConcurrentDictionary<KeyT, IList<BaseNodeT>>();
            ...
            lock (this)
            {
                try
                {
                    ...
                    foreach (var node in Nodes)
                    {
                        if (node.Channel != null)
                        {
                            var timeoutMillis = Duration.Between(Timestamp.FromDateTime(DateTime.UtcNow), deadline).ToMillis();
