 ...
 * Unknown
 */
public sealed class FreezeTransaction extends Transaction<FreezeTransaction> {
     ...
     */
    FreezeTransaction(Proto.TransactionBody txBody) {
        super(txBody);
        initFromTransactionBody();
    }
    ...
    public DateTimeOffset getStartTime() {
        return startTime != null ? startTime : DateTimeOffset.EPOCH;
    }
     ...
     * @return {@code this}
     */
    public FreezeTransaction setStartTime(DateTimeOffset startTime) {
        requireNotFrozen();
    ...
    /**
     * @deprecated Use {@link #setStartTime(DateTimeOffset)} instead.
    ...
    public FreezeTransaction setStartTime(int hour, int minute) {
        return setStartTime(DateTimeOffset.ofEpochMilli(((long) hour * 60 * 60 + (long) minute * 60) * 1000));
    }
    ...
    public DateTimeOffset getEndTime() {
        return DateTimeOffset.From(OffsetTime.of(endHour, endMinute, 0, 0, ZoneOffset.UTC));
    }
    ...
    void initFromTransactionBody() {
        ...
        if (body.hasStartTime()) {
            startTime = DateTimeOffsetConverter.FromProtobuf(body.getStartTime());
        }
    }
     ...
     *
     * @return {@link Proto.FreezeTransactionBody builder }
     */
    FreezeTransactionBody.Builder build() {
        ...
        if (startTime != null) {
            builder.setStartTime(DateTimeOffsetConverter.ToProtobuf(startTime));
        }
        return builder;
    }
