 ...
 */
	public abstract class Transaction<T> : Executable<T, Proto.Transaction, Proto.TransactionResponse, TransactionResponse> 
	{
		 ...
		 */
		protected static readonly AccountId DUMMY_ACCOUNT_ID = new AccountId(0L, 0L, 0L);

		/**
		 * Dummy transaction ID used to assist in deserializing incomplete Transactions.
		 */
		protected static readonly TransactionId DUMMY_TRANSACTION_ID = TransactionId.WithValidStart(DUMMY_ACCOUNT_ID, DateTimeOffset.EPOCH);
		 ...
		 * response, we try a different transaction and thus a different node.
		 */
		protected List<Proto.SignedTransaction.Builder> innerSignedTransactions =
		...
		Transaction(LinkedHashMap<TransactionId, LinkedHashMap<AccountId, Proto.Transaction>> txs)
		{
			...
			if (!transactionMap.isEmpty()
				...
				var signedTransaction = SignedTransaction.Parser.ParseFrom(
				...
				for (var transactionEntry : txs.entrySet())
				{
					...
					for (var nodeEntry : transactionEntry.getValue().entrySet())
					{
						...
						var transaction =
								SignedTransaction.Parser.ParseFrom(nodeEntry.getValue().getSignedTransactionBytes());
	...
	@Override
	ExecutionState getExecutionState(Status status, Proto.TransactionResponse response) {
