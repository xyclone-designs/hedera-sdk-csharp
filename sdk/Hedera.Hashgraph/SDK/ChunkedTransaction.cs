...
abstract class ChunkedTransaction<T extends ChunkedTransaction<T>> extends Transaction<T> {
     ...
     */
    ChunkedTransaction(Proto.TransactionBody txBody) {
        super(txBody);
    }
     ...
     */
    public readonly List<Dictionary<AccountId, byte[]>> getAllTransactionHashesPerNode() {
        if (!this.isFrozen()) {
            throw new IllegalStateException(
        ...
        var transactionHashes = new ArrayList<Dictionary<AccountId, byte[]>>(txCount);
     ...
     */
    public List<Dictionary<AccountId, Dictionary<PublicKey, byte[]>>> getAllSignatures() {
        ...
        var retval = new ArrayList<Dictionary<AccountId, Dictionary<PublicKey, byte[]>>>(txCount);
     ...
     */
    public List<TransactionResponse> executeAll(Client client) , TimeoutException {
     ...
     */
    public Task<List<TransactionResponse>> executeAllAsync(Client client) {
     ...
     * @return Future result of execution for each chunk
     */
    public Task<List<TransactionResponse>> executeAllAsync(Client client, Duration timeoutPerChunk) {
        freezeAndSign(client);

        Task<List<TransactionResponse>> future =
                Task.supplyAsync(() -> new ArrayList<>(transactionIds.size()));
    ...
    int getRequiredChunks() {
        ...
        if (requiredChunks > maxChunks) {
            throw new ArgumentException("message of " + this.data.size() + " bytes requires " + requiredChunks
