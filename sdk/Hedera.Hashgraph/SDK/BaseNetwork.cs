 ...
 */
    abstract class BaseNetwork<BaseNetworkT, KeyT, BaseNodeT> where BaseNetworkT : BaseNetwork<BaseNetworkT, KeyT, BaseNodeT> where BaseNodeT : BaseNode<BaseNodeT, KeyT>
	{
         ...
         */
		protected Dictionary<KeyT, List<BaseNodeT>> network = new ConcurrentHashMap<>();
        ...
        protected BaseNetwork(ExecutorService executor) {
            ...
            earliestReadmitTime = DateTimeOffset.now().plus(minNodeReadmitTime);
        }
        ...
        public synchronized void setMinNodeReadmitTime(Duration minNodeReadmitTime) {
            ...
            for (var node : nodes) {
                node.readmitTime = DateTimeOffset.now();
            }
        }
         ...
         *
         * Descending order is important here because {@link BaseNetwork#setNetwork(Dictionary<string, KeyT>)} uses a for-each loop.
         *
         * @param network - the new network
         * @return - list of indexes in descending order
         */
        protected List<Integer> getNodesToRemove(Dictionary<string, KeyT> network) {
         ...
         */
        synchronized BaseNetworkT setNetwork(Dictionary<string, KeyT> network) {
            ...
            for (var index : getNodesToRemove(network)) {
                var stopAt = DateTimeOffset.now().getEpochSecond() + closeTimeout.getSeconds();
                var remainingTime = stopAt - DateTimeOffset.now().getEpochSecond();
        ...
        private bool addressIsInNodeList(string addressString, List<BaseNodeT> nodes) {
         ...
         */
        protected synchronized List<BaseNodeT> getNumberOfMostHealthyNodes(int count)  {
            readmitNodes();
            removeDeadNodes();
        ...
        synchronized Throwable awaitClose(DateTimeOffset deadline, @Nullable Throwable previousError) {
            try {
                ...
                for (var node : nodes) {
                    if (node.channel != null) {
                        var timeoutMillis =
                                Duration.between(DateTimeOffset.now(), deadline).toMillis();
