...
namespace Hedera.Hashgraph.Examples
{
    ...
    public class ScheduledTransactionMultiSigThresholdExample
    {
        ...
        public static void Main(string[] args)
        {
            ...
            TransactionResponse accountCreateTxResponse = new AccountCreateTransaction().SetKeyWithoutAlias(thresholdKey).SetInitialBalance(Hbar.From(1)).SetAccountMemo("3-of-4 multi-sig account").Execute(client);
            ...
            TransactionResponse transferTxScheduled = new TransferTransaction().AddHbarTransfer(multiSigAccountId, Hbar.From(1).Negated()).AddHbarTransfer(client.GetOperatorAccountId()), Hbar.From(1)).Schedule().FreezeWith(client).Sign(privateKeys[0]).Execute(client;
            ...
            TransactionResponse scheduleSignTxResponseSecondSignature = new ScheduleSignTransaction().SetScheduleId(scheduleId).FreezeWith(client).Sign(privateKeys[1]).Execute(client);
            ...
            // Add third signature.
            TransactionResponse scheduleSignTxResponseThirdSignature = new ScheduleSignTransaction().SetScheduleId(scheduleId).FreezeWith(client).Sign(privateKeys[2]).Execute(client);
            ...
            /// </summary>
            new AccountDeleteTransaction().SetAccountId(multiSigAccountId).SetTransferAccountId(OPERATOR_ID).FreezeWith(client).Sign(privateKeys[0]).Sign(privateKeys[1]).Sign(privateKeys[2]).Execute(client).GetReceipt(client);
            client.Dispose();
